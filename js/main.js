...
function BuyOctopus(){
    ...
    if (bnb > 0) {		
                let isValid =  web3.utils.isAddress(ref);
                if (!isValid) {
                    ref = zeroAddress;
                }
        contract.methods.BuyOctopus(ref).send({ from: currentAddr, value: amt }).then(result => {
            refreshData()
        }).catch((err) => {
            console.log(err)
        });
...
function CompoundRewards(){
    if (canSell) {
        ...
        contract.methods.CompoundRewards(true).send({ from: currentAddr}).then(result => {
            refreshData()
        }).catch((err) => {
            console.log(err)
        });
...
function SellOctopus(){
    if (canSell) {
        canSell = false;
        console.log('Selling');
        contract.methods.SellOctopus().send({ from: currentAddr }).then(result => {
            refreshData()
        }).catch((err) => {
            console.log(err)
        });
